...
public class TripInfoView {
	...
	private DefaultTableModel tripModel2;
	private JTable tripTable2;
	private JScrollPane tripPan2;
	private String[] title2 = { "메인타이틀", "서브타이틀", "기간"};
	/** 검색 조건 */
	private String key;
	...
	public TripInfoView() {
		...
		System.out.println(festivalService.getFestivals());
		/* 메인 화면 설정 */
		frame = new JFrame("Enjoy! Trip - 즐거운 여행");
		frame2 = new JFrame("근처 관광 정보");
...
//		});
	...
	private void showTripInfo(int num) {
		...
		if (curTrip.getImg() != null && curTrip.getImg().trim().length() != 0) {
			...
			if(num <= 8) {
				icon = new ImageIcon("img/"+ "image0" + (num+1) + ".jpg");
			}else if(num >= 9 && num <= 10) {
				icon = new ImageIcon("img/"+ "image" + (num+1) + ".jpg");
			}else {
	...
	private void showTourismInfo(int num) {
		// 파싱을 해서 -> 
		curTrip = tripService.search(num);
		String add = curTrip.getStreetAddress();
	...
	public void setMain() {
		...
		tripTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				// 클릭시 해당 지역의 축제 정보를 별도의 어플리케이션에 보여준다.
				showTourismInfo(code);
			}
		});
